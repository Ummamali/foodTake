    ...
    <Modal active={props.active} cancel={props.cancel}>
        <div className='small-card p-6 rounded'>
            ...
            <div className='flex flex-row-reverse mt-4'>
                <button className='btn bg-primary-light text-white text-opacity-70 tracking-wide rounded font-light'>Order</button>
                <button className='btn text-gray-600' onClick={props.cancel}>Cancel</button>
            </div>
        </div>
    </Modal>
    );
...
function BillItem(props){
    return (
        <div className='bill-item flex items-center justify-between line-down py-3 px-2'>
            <div>
                <h2 className='text-2xl text-gray-900 leading-none mb-4'>{props.name}</h2>
            ...
            <div className='flex items-center'>
                <div className='border border-gray-400 p-2 leading-none rounded-sm text-gray-700 font-bold mr-12'>
                    <p><small>x</small>{props.amount}</p>
                </div>
